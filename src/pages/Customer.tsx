...
export default function CustomerPage() {
  ...
  return (
    <div className="animate-fade-in flex gap-4 h-full">
      ...
      <div className="w-64 flex-shrink-0 flex flex-col gap-3 h-[calc(100vh-140px)]">
        <button
        ...
        <div className="category-panel flex-1 overflow-hidden flex flex-col">
          <div className="p-3 border-b border-slate-100 dark:border-dark-700/50">
            <h2 className="text-[10px] font-black text-slate-400 dark:text-dark-500 uppercase tracking-[0.2em] mb-2">Customer Base</h2>
            <div className="relative">
              <SearchBar value={custSearch} onChange={setCustSearch} placeholder="Search customers..." fullWidth={true} className="!py-1.5 !text-xs" />
            </div>
          </div>

          <div className="flex-1 overflow-y-auto custom-scrollbar p-1 space-y-1">
            {filteredCustomers.length === 0 ? (
              <div className="px-4 py-8 text-center text-xs text-slate-400 dark:text-dark-500 italic">
                No customers found
              </div>
            ) : (
              filteredCustomers.map((c) => (
                <div
                  ...
                  className={selectedCust === c.id ? 'category-item-active' : 'category-item-inactive'}
                >
                  <div className="flex items-start gap-2.5 min-w-0">
                    <div className={`mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 ${selectedCust === c.id ? 'bg-primary-600 animate-pulse' : 'bg-slate-300 dark:bg-dark-600'}`}></div>
            ...
            <div className="glass rounded-xl overflow-hidden" ref={printRef}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {paged.length === 0 ? (
                      <tr><td colSpan={6} className="table-cell text-center text-slate-400 dark:text-dark-500 py-12">No transactions yet</td></tr>
                    ) : paged.map((e) => (
                  ...
                  {paged.length > 0 && (
                    <tfoot className="bg-slate-50/50 dark:bg-dark-800/50 font-semibold border-t border-slate-200 dark:border-dark-700/50">
                      <tr>
                        ...
                        <td className="table-cell text-right text-slate-900 dark:text-white">₨ {formatCurrency(pageTotals.debit - pageTotals.credit)}</td>
