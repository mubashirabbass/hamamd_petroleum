...
export default function LedgerPage() {
  ...
  }, [ledgerEntries, selectedCat, search]);
  ...
  const handleAddCat = (e: React.FormEvent) => {
    ...
    addLedgerCategory(catName.trim());
    toast('Ledger category created', 'success');
    ...
    <div className="animate-fade-in flex gap-4 h-full">
      ...
      <div className="w-52 flex-shrink-0 flex flex-col gap-2">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => setShowCatForm(true)} className="p-1.5 rounded-lg bg-primary-600/10 dark:bg-primary-600/20 text-primary-600 dark:text-primary-400 hover:bg-primary-600/20 dark:hover:bg-primary-600/30 transition-colors">
            <FolderPlus className="w-4 h-4" />
          </button>
        </div>
        {ledgerCategories.length === 0 && (
          <p className="text-xs text-slate-400 dark:text-dark-500 text-center mt-4">No categories yet.<br />Create one to start.</p>
        )}
        {ledgerCategories.map((c) => (
          <div
            key={c.id}
            onClick={() => { setSelectedCat(c.id); setSearch(''); setPage(1); }}
            className={`flex items-center justify-between px-3 py-2.5 rounded-lg cursor-pointer text-sm transition-all group ${selectedCat === c.id ? 'bg-primary-50 dark:bg-primary-600/20 text-primary-600 dark:text-primary-400 border border-primary-200 dark:border-primary-500/20' : 'text-slate-500 dark:text-dark-400 hover:bg-slate-100 dark:hover:bg-dark-700/50 hover:text-slate-900 dark:hover:text-dark-100'}`}
          >
            <span className="truncate font-medium">{c.name}</span>
            <button
              onClick={(e) => { e.stopPropagation(); deleteLedgerCategory(c.id); if (selectedCat === c.id) setSelectedCat(null); toast('Category deleted', 'warning'); }}
              className="opacity-0 group-hover:opacity-100 text-slate-400 dark:text-dark-500 hover:text-red-500 dark:hover:text-red-400 transition-all"
            >
      ...
      <div className="flex-1 min-w-0">
        {showCatForm && (
          <Modal title="New Ledger Category" onClose={() => setShowCatForm(false)}>
            <form onSubmit={handleAddCat} className="space-y-3">
              <div><label className="label">Category Name *</label><input className="input" value={catName} onChange={(e) => setCatName(e.target.value)} placeholder="e.g. Bank Account, Cash" required /></div>
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-violet-600/10 dark:bg-violet-600/20 flex items-center justify-center"><BookOpen className="w-5 h-5 text-violet-600 dark:text-violet-400" /></div>
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">Ledger</h1>
              {cat && <span className="text-xs text-slate-500 dark:text-dark-400">{cat.name}</span>}
            </div>
          </div>
        ...
        {selectedCat ? (
          <>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: 'Total Debit', value: totals.debit, color: 'text-red-600 dark:text-red-400' },
                { label: 'Total Credit', value: totals.credit, color: 'text-emerald-600 dark:text-emerald-400' },
                { label: 'Net Balance', value: totals.debit - totals.credit, color: 'text-primary-600 dark:text-primary-400' },
              ].map((s) => (
            ...
            <div className="glass rounded-xl overflow-hidden">
              <div className="p-4 border-b border-slate-200 dark:border-dark-700/50">
                    ...
                    {paged.length === 0 ? (
                      ...
                      <tr key={e.id} className="table-row">
                        ...
                        <td className="table-cell text-right">
                          <button onClick={() => { deleteLedgerEntry(e.id); toast('Entry deleted', 'warning'); }} className="text-slate-400 dark:text-dark-500 hover:text-red-600 dark:hover:text-red-400 transition-colors p-1"><Trash2 className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 dark:text-dark-500">
            <BookOpen className="w-12 h-12 mb-3 opacity-20 dark:opacity-30" />
