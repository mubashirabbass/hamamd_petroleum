...
import { Plus, Trash2, BookOpen, Eye, Edit2 } from 'lucide-react';
...
export default function LedgerPage() {
  const { 
    ...
    addLedgerEntry, deleteLedgerEntry, settings, currentUser 
  } = useStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingEntity) {
      useStore.getState().updateLedgerEntry(editingEntity.id, payload);
  ...
  const pageTotals = useMemo(() => ({
    debit: paged.reduce((s, e) => s + e.debit, 0),
    credit: paged.reduce((s, e) => s + e.credit, 0),
  }), [paged]);
  ...
  return (
    <div className="animate-fade-in flex gap-4 h-full">
        ...
        <div className="flex flex-col h-full bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-700/50 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="smart-scroll flex-1 p-2 space-y-1">
            {ledgerCategories.length === 0 ? (
              <div className="p-8 text-center text-xs text-slate-400 italic">No departments registered</div>
            ) : (
              ledgerCategories.map((c) => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedCat === c.id 
                      ? 'bg-primary-600/5 text-primary-600 border-primary-600/10 shadow-sm shadow-primary-600/5 relative overflow-hidden' 
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {cat && <span className="text-xl md:text-2xl font-black text-primary-600 dark:text-primary-500 tracking-widest uppercase block mt-1 drop-shadow-sm">{cat.name}</span>}
            </div>
          </div>
          {selectedCat && (
        ...
        {selectedCat ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="glass p-5 rounded-2xl border-l-4 border-red-500 shadow-sm translate-y-0 hover:-translate-y-1 transition-all duration-300">
                <p className="text-[10px] font-black text-slate-400 dark:text-dark-500 uppercase tracking-widest mb-1">Total Debit</p>
                <p className="text-2xl font-black text-red-600 dark:text-red-400 tabular-nums">₨ {formatCurrency(totals.debit)}</p>
              </div>
              <div className="glass p-5 rounded-2xl border-l-4 border-emerald-500 shadow-sm translate-y-0 hover:-translate-y-1 transition-all duration-300">
              ...
              <div className="glass p-5 rounded-2xl border-l-4 border-primary-500 shadow-sm translate-y-0 hover:-translate-y-1 transition-all duration-300">
                <p className="text-[10px] font-black text-slate-400 dark:text-dark-500 uppercase tracking-widest mb-1">Net Balance</p>
                <p className="text-2xl font-black text-primary-600 dark:text-primary-400 tabular-nums">₨ {formatCurrency(totals.debit - totals.credit)}</p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-dark-800/50">
                    {paged.length === 0 ? (
                      <tr><td colSpan={6} className="table-cell text-center text-slate-400 dark:text-dark-500 py-12 italic">No transactions found for this period</td></tr>
                    ) : paged.map((e) => (
                      <tr key={e.id} className="table-row group hover:bg-slate-50 dark:hover:bg-dark-800/50">
                        ...
                        <td className="table-cell text-right text-red-600 dark:text-red-400 font-mono text-xs italic">{e.debit ? `₨ ${formatCurrency(e.debit)}` : '—'}</td>
                          ...
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {currentUser?.role === 'Admin' && (
                              <>
                                ...
                                <button onClick={() => { if(confirm('Delete this entry?')) { deleteLedgerEntry(e.id); toast('Entry deleted', 'warning'); } }} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors" title="Delete Entry">
                  ...
                  {paged.length > 0 && (
                    <tfoot className="bg-slate-50/50 dark:bg-dark-800/50 font-bold border-t border-slate-200 dark:border-dark-700/50">
                      <tr>
                        <td colSpan={2} className="table-cell text-right text-[10px] uppercase tracking-wider text-slate-400">Page Calculations:</td>
                        <td className="table-cell text-right text-red-600 dark:text-red-400 tabular-nums text-xs">₨ {formatCurrency(pageTotals.debit)}</td>
                        <td className="table-cell text-right text-emerald-600 dark:text-emerald-400 tabular-nums text-xs">₨ {formatCurrency(pageTotals.credit)}</td>
                        <td className="table-cell text-right text-primary-600 dark:text-primary-400 tabular-nums text-sm">₨ {formatCurrency(pageTotals.debit - pageTotals.credit)}</td>
          ...
          <div className="flex flex-col items-center justify-center h-[50vh] text-slate-400 dark:text-dark-500 animate-fade-in opacity-40">
            ...
            <p className="font-medium">Select a department from the left to view ledger</p>
          </div>
        )}
      </div>
      ...
      <ManageCategoriesModal
        ...
        title="Manage Ledger Departments"
        categories={ledgerCategories}
        onAdd={addLedgerCategory}
        onUpdate={updateLedgerCategory}
        onDelete={(id) => { deleteLedgerCategory(id); if (selectedCat === id) setSelectedCat(null); }}
      />
    </div>
  );
}
