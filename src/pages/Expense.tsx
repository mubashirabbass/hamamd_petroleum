...
export default function ExpensePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return expenseEntries
      ...
      .filter((e) => !search || e.details.toLowerCase().includes(search.toLowerCase()) || e.date.includes(search));
  }, [expenseEntries, selectedCat, search]);
  ...
  return (
    <div className="animate-fade-in flex gap-4 h-full">
      ...
      <div className="w-52 flex-shrink-0">
        ...
        {expenseCategories.length === 0 && <p className="text-xs text-slate-400 dark:text-dark-500 text-center mt-4">No categories yet.</p>}
        {expenseCategories.map((c) => (
          <div key={c.id} onClick={() => { setSelectedCat(c.id); setSearch(''); setPage(1); }}
            ...
            <button onClick={(e) => { e.stopPropagation(); deleteExpenseCategory(c.id); if (selectedCat === c.id) setSelectedCat(null); toast('Category deleted', 'warning'); }}
          ...
          <>
            ...
            <div className="glass rounded-xl overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-dark-700/50">
                ...
                <span className="text-sm text-slate-500 dark:text-dark-400">Page Total: <span className="text-red-600 dark:text-red-400 font-semibold">₨ {formatCurrency(pageTotal)}</span></span>
              </div>
              <table className="w-full">
                ...
                <tbody>
                  {paged.length === 0 ? <tr><td colSpan={4} className="table-cell text-center text-slate-400 dark:text-dark-500 py-12">No expenses yet</td></tr>
                    : paged.map((e) => (
                      <tr key={e.id} className="table-row">
                        <td className="table-cell">{formatDate(e.date)}</td>
                        <td className="table-cell text-slate-600 dark:text-dark-300">{e.details || '—'}</td>
