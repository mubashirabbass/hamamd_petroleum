...
export default function AssetPage() {
  ...
  const preFilterBalance = useMemo(() => {
    ...
    const allEntries = assetEntries.filter(e => e.categoryId === selectedCat);
    if (!settings.startDate) return 0;
    return allEntries
      .filter(e => e.date < settings.startDate)
      .reduce((sum, e) => sum + (e.debit || 0) - (e.credit || 0), 0);
  }, [assetEntries, selectedCat, settings.startDate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = window.confirm(editingEntity ? 'Confirm Update: Save changes to this asset?' : 'Confirm Save: Register this new asset entry?');
    if (!confirmed) return;
  ...
  const globalDashboardTotals = useMemo(() => {
    ...
    assetCategories.forEach(cat => {
      ...
      const allC = assetEntries.filter(e => e.categoryId === cat.id);
      let pre = 0;
  ...
  }, [assetCategories, assetEntries, settings.startDate, fromDate, toDate]);
      ...
      <div className="p-4 bg-white dark:bg-dark-900/50 border-b border-slate-200 dark:border-dark-800 flex-shrink-0">
        <div className="pill-nav-container">
          <button
            ...
            className={cn("pill-nav-item", activeTab === 'dashboard' ? "pill-nav-item-active bg-emerald-600 border-emerald-600 shadow-emerald-500/30" : "hover:border-emerald-100")}
          >
            Analytics
          </button>
          <button
            onClick={() => setActiveTab('database')}
            className={cn("pill-nav-item", activeTab === 'database' ? "pill-nav-item-active bg-emerald-600 border-emerald-600 shadow-emerald-500/30" : "hover:border-emerald-100")}
          >
            Entries
          </button>
          <button
            onClick={() => setActiveTab('register')}
            className={cn("pill-nav-item", activeTab === 'register' ? "pill-nav-item-active bg-emerald-600 border-emerald-600 shadow-emerald-500/30" : "hover:border-emerald-100")}
          >
            New Asset
          </button>
          <button
            onClick={() => setActiveTab('manage')}
            className={cn("pill-nav-item", activeTab === 'manage' ? "pill-nav-item-active bg-emerald-600 border-emerald-600 shadow-emerald-500/30" : "hover:border-emerald-100")}
          >
            Settings
          </button>
        </div>
        ...
        {activeTab === 'dashboard' ? (
          <div className="flex-1 flex flex-col h-full overflow-hidden p-4 md:p-6 pb-6">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
              <div className="glass p-4 rounded-3xl border-l-4 border-emerald-600 shadow-xl bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-900/10 dark:to-dark-900 col-span-2 md:col-span-1">
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">Net Valuation</p>
              ...
              <div className="glass p-3 rounded-2xl border-l-4 border-slate-400 shadow-sm bg-white dark:bg-dark-800/50 col-span-1">
                ...
                <p className="text-sm font-black text-slate-900 dark:text-white">{assetCategories.length}</p>
              </div>
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-slate-50 dark:bg-dark-950/20">
            ...
            <div className="bg-white dark:bg-dark-900 border-b border-slate-200 dark:border-dark-800 p-3 flex items-center gap-3 shrink-0 w-full overflow-hidden">
               <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-dark-800 rounded-2xl border border-slate-200 dark:border-dark-700 focus-within:ring-2 focus-within:ring-emerald-600/20 focus-within:border-emerald-600 transition-all shrink-0">
                   ...
                   <button 
                     ...
                     className={cn(
                       ...
                       selectedCat === c.id 
                         ...
                         : "bg-white dark:bg-dark-900 border-slate-100 dark:border-dark-800 text-slate-600 dark:text-dark-400 hover:border-emerald-200 hover:shadow-md"
                     )}
                   >
                     {c.name}
                   </button>
