...
export default function AssetPage() {
  ...
  const filteredSidebar = useMemo(() =>
    assetCategories.filter((c) => !search || c.name.toLowerCase().includes(search.toLowerCase())),
    [assetCategories, search]
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingEntity) {
      updateAssetEntry(editingEntity.id, payload);
                                ...
                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  ...
                                  {currentUser?.role === 'Admin' && (
                                    <>
                                      ...
                                      <button onClick={() => { if(confirm('Delete entry?')) { deleteAssetEntry(e.id); toast('Entry deleted', 'warning'); } }} className="text-slate-300 hover:text-red-600 transition-colors p-1.5 hover:bg-red-50 rounded-lg">
                             ...
                             <>
                               ...
                               <td className="px-6 py-4 text-right">
                                  <div className="flex items-center gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                     ...
                                     {currentUser?.role === 'Admin' && (
                                       <button onClick={() => { if(confirm('Delete account and all history?')) deleteAssetCategory(c.id); }} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl"><Trash2 className="w-4 h-4" /></button>
                                     )}
                                  </div>
                               </td>
                             </>
        ...
        <Modal title={editingEntity ? `Edit Entry — ${cat?.name}` : `Add Entry — ${cat?.name}`} onClose={closeForm}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" className="btn-primary !bg-emerald-600"><Plus className="w-4 h-4" />{editingEntity ? 'Update' : 'Add'} Entry</button>
            </div>
          </form>
        </Modal>
      )}
