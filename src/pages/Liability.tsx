...
import { Plus, Landmark, Trash2, Eye, Edit2 } from 'lucide-react';
...
export default function LiabilityPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingEntity) {
      useStore.getState().updateLiabilityEntry(editingEntity.id, payload);
      toast('Liability entry updated', 'success');
    } else {
  ...
  return (
    <div className="animate-fade-in flex gap-4 h-full">
      ...
      <div className="flex-1 min-w-0">
        {showEntryForm && (
          <Modal title={editingEntity ? `Edit Liability Entry — ${cat?.name}` : `Add Liability Entry — ${cat?.name}`} onClose={closeForm}>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" className="btn-primary flex items-center gap-2"><Plus className="w-4 h-4" /> {editingEntity ? 'Update Liability' : 'Add Liability'} </button>
              </div>
            </form>
          </Modal>
        )}
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-orange-600/10 dark:bg-orange-600/20 flex items-center justify-center">
              <Landmark className="w-8 h-8 text-orange-600 dark:text-orange-400" />
            </div>
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
                {cat?.name || 'Liabilities (Loans)'}
              </h1>
              {cat && <span className="text-sm font-black text-orange-600 dark:text-orange-500 tracking-widest uppercase block mt-1 drop-shadow-sm">LIABILITY ACCOUNT</span>}
            </div>
          </div>
          {selectedCat && (
            <button onClick={() => { closeForm(); setShowEntryForm(true); }} className="btn-primary !bg-orange-600 hover:!bg-orange-500 flex items-center gap-2">
                          ...
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {currentUser?.role === 'Admin' && (
                              <>
                                ...
                                <button onClick={() => { if(confirm('Delete this entry?')) { deleteLiabilityEntry(e.id); toast('Entry deleted', 'warning'); } }} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors" title="Delete Entry">
