...
export default function LiabilityPage() {
  const { liabilityCategories, liabilityEntries, addLiabilityCategory, deleteLiabilityCategory, addLiabilityEntry, deleteLiabilityEntry } = useStore();
  ...
  }, [liabilityEntries, selectedCat, search]);
  ...
  const handleAddCat = (e: React.FormEvent) => {
    ...
    addLiabilityCategory(catName.trim()); toast('Liability category created', 'success'); setCatName(''); setShowCatForm(false);
  };

  const handleAddEntry = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCat || !form.date) { toast('Fill required fields', 'error'); return; }
    addLiabilityEntry({ categoryId: selectedCat, date: form.date, description: form.description, debit: parseFloat(form.debit) || 0, credit: parseFloat(form.credit) || 0, balance: 0 });
  ...
  return (
    <div className="animate-fade-in flex gap-4 h-full">
      <div className="w-52 flex-shrink-0">
        ...
        {liabilityCategories.length === 0 && <p className="text-xs text-slate-400 dark:text-dark-500 text-center mt-4">No categories yet.</p>}
        {liabilityCategories.map((c) => (
          <div key={c.id} onClick={() => { setSelectedCat(c.id); setSearch(''); setPage(1); }}
            ...
            <button onClick={(e) => { e.stopPropagation(); deleteLiabilityCategory(c.id); if (selectedCat === c.id) setSelectedCat(null); toast('Deleted', 'warning'); }}
      ...
      <div className="flex-1 min-w-0">
        {showCatForm && <Modal title="New Liability Category" onClose={() => setShowCatForm(false)}><form onSubmit={handleAddCat} className="space-y-3"><div><label className="label">Category Name *</label><input className="input" value={catName} onChange={(e) => setCatName(e.target.value)} placeholder="e.g. Bank Loan, Payables" required /></div><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowCatForm(false)} className="btn-secondary">Cancel</button><button type="submit" className="btn-primary"><Plus className="w-4 h-4" />Create</button></div></form></Modal>}
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-orange-600/10 dark:bg-orange-600/20 flex items-center justify-center"><AlertTriangle className="w-5 h-5 text-orange-600 dark:text-orange-400" /></div>
            <div><h1 className="text-xl font-bold text-slate-900 dark:text-white">Liability</h1>{cat && <span className="text-xs text-slate-500 dark:text-dark-400">{cat.name}</span>}</div>
          </div>
        ...
        {selectedCat ? (
          <>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[{ label: 'Total Debit', value: totals.debit, color: 'text-red-600 dark:text-red-400' }, { label: 'Total Credit', value: totals.credit, color: 'text-emerald-600 dark:text-emerald-400' }, { label: 'Net Liability', value: totals.credit - totals.debit, color: 'text-orange-600 dark:text-orange-400' }].map((s) => (
            ...
            <div className="glass rounded-xl overflow-hidden">
              <div className="p-4 border-b border-slate-200 dark:border-dark-700/50"><SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search liabilities..." /></div>
                  ...
                  {paged.length === 0 ? <tr><td colSpan={6} className="table-cell text-center text-slate-400 dark:text-dark-500 py-12">No entries yet</td></tr>
                    : paged.map((e) => (
                      <tr key={e.id} className="table-row">
                        ...
                        <td className="table-cell text-right"><button onClick={() => { deleteLiabilityEntry(e.id); toast('Entry deleted', 'warning'); }} className="text-slate-400 dark:text-dark-500 hover:text-red-500 dark:hover:text-red-400 transition-colors p-1"><Trash2 className="w-4 h-4" /></button></td>
                      </tr>
                    ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 dark:text-dark-500"><AlertTriangle className="w-12 h-12 mb-3 opacity-20 dark:opacity-30" /><p>Select a category from the left</p></div>
        )}
      </div>
    </div>
  );
