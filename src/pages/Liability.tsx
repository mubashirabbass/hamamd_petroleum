...
import { Landmark, Plus, Trash2, Eye, Edit2, Search, Check, X, FileText, Settings, UserPlus, Printer, BarChart3, ArrowRight, ArrowUpDown, Save } from 'lucide-react';
    ...
    <div className="animate-fade-in space-y-4 flex flex-col h-full overflow-hidden">
                    ...
                    {(() => {
                          ...
                          const sorted = [...filtered].sort((a, b) => {
                            switch (dashSort) {
                              case 'name_asc':  return a.name.localeCompare(b.name);
                              case 'name_desc': return b.name.localeCompare(a.name);
                              case 'debt_desc': return a.balance - b.balance; // Balance is debit-credit. Liability is credit-debit usually. But if debit > credit, it's a negative debt. 
                                                                             // For liability, balance is normally credit-debit. 
                                                                             // Looking at balance calculation: balance = debit - credit. 
      ...
      {showReport && cat && (
        <PrintReportModal
          ...
          type="liability"
          title={`${cat.name} Liability Statement`}
          columns={[
            { header: 'Date', accessor: 'date' },
            { header: 'Description', accessor: 'description' },
            { header: 'Debit (Paid)', accessor: 'debit', align: 'right', isCurrency: true },
            { header: 'Credit (Owed)', accessor: 'credit', align: 'right', isCurrency: true },
            { header: 'Balance', accessor: 'balance', align: 'right', isCurrency: true },
          ]}
          data={withBalance}
          totals={{
            debit: totals.debit,
      ...
      {showSummaryReport && (
        <PrintReportModal
          ...
          columns={[
            { header: 'Liability Account', accessor: 'name' },
            { header: 'Entries', accessor: 'count', align: 'center' },
            { header: 'Current Liability', accessor: 'balance', align: 'right', isCurrency: true },
          ]}
          data={(() => {
            const items = liabilityCategories.map(cat => {
              const entries = filterByStartDate(liabilityEntries, settings.startDate)
                .filter(e => e.categoryId === cat.id)
                .filter(e => {
                  const matchesFrom = !fromDate || e.date >= fromDate;
              ...
              const debit = entries.reduce((sum, e) => sum + (e.debit || 0), 0);
              const credit = entries.reduce((sum, e) => sum + (e.credit || 0), 0);
              return { ...cat, balance: debit - credit, count: entries.length };
            });
          ...
          totals={{
            balance: (() => {
              ...
              const filteredCats = liabilityCategories.filter(c => !dashboardSearch || c.name.toLowerCase().includes(dashboardSearch.toLowerCase()));
              ...
              const debit = periodEntries.reduce((sum, e) => sum + e.debit, 0);
              const credit = periodEntries.reduce((sum, e) => sum + e.credit, 0);
              return debit - credit;
            })()
          }}
          dateRange={fromDate || toDate ? { from: fromDate, to: toDate } : undefined}
              ...
              <div className="desktop-form-row !border-b-0">
                ...
                <div className="desktop-form-field">
                  <input 
                    type="number" 
                    step="any" 
                    className="input !py-1.5 !bg-red-50/30 dark:!bg-red-900/10 focus:ring-red-500/20" 
                    placeholder="0.00"
                    value={form.credit} 
            ...
            <div className="desktop-form-footer">
              <div className="desktop-summary-strip flex-1 mr-4">
                <div className="flex flex-col">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Balance Change</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="submit" disabled={isSaving} className="btn-primary !bg-orange-600 hover:opacity-90 !py-2 !px-6">
