        ...
        {showForm && (
          <Modal title={editingEntity ? `Edit ${fuelType} Sale` : `Add ${fuelType} Sale`} onClose={closeForm}>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div><label className="label">Rate (Γé¿) *</label><input type="number" step="0.01" className="input" value={form.rate} onChange={(e) => set('rate', e.target.value)} required /></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="table-header">
                ...
                <th className="table-cell text-right">Rate (Γé¿)</th>
                <th className="table-cell text-right">Amount (Γé¿)</th>
              ...
              <tbody>
                {paged.length === 0 ? (
                  ...
                  <tr key={s.id} className="table-row group hover:bg-slate-50 dark:hover:bg-dark-800/50">
                    ...
                    <td className="table-cell text-right">Γé¿ {formatCurrency(s.rate)}</td>
                    <td className="table-cell text-right font-semibold text-slate-900 dark:text-white">Γé¿ {formatCurrency(s.amount)}</td>
              ...
              {paged.length > 0 && (
                <tfoot className="bg-slate-50/50 dark:bg-dark-800/50 font-semibold border-t-[3px] border-double border-slate-300 dark:border-dark-600">
                  <tr>
                    ...
                    <td className="table-cell text-right text-slate-900 dark:text-white font-bold py-3">ΓÇö</td>
                    <td className="table-cell text-right text-emerald-600 dark:text-emerald-400 font-bold py-3">Γé¿ {formatCurrency(pageTotals.amount)}</td>
