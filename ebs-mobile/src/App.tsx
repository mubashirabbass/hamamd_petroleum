...
export default function App() {
  ...
  // ── Zoom — CSS transform:scale on #root works in Android WebView & all Tauri targets ──
  const applyZoom = (zoom: number) => {
    const root = document.getElementById('root');
    if (root) {
      if (zoom === 1.0) {
        // Reset completely — no transform overhead
        root.style.transform = '';
        root.style.transformOrigin = '';
        root.style.width = '';
        root.style.height = '';
      } else {
        // Scale from top-left; expand the logical size so content fills the screen
        root.style.transformOrigin = 'top left';
        root.style.transform = `scale(${zoom})`;
        // Compensate: make the element larger in logical pixels so after scaling it fills the viewport
