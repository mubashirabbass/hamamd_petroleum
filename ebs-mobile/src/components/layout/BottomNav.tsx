...
export default function BottomNav({ onMore }: { onMore?: () => void }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-20 bg-white dark:bg-dark-900 border-t border-slate-200/60 dark:border-dark-800 flex items-center justify-around px-2 z-[150] shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
      {navItems.map((item) => {
        ...
        return (
          <NavLink
            ...
            className={cn(
              ...
              active ? "text-primary-600 dark:text-primary-400" : "text-slate-400 dark:text-dark-400"
            )}
          >
            <div className={cn(
              "p-2 rounded-2xl transition-all duration-500",
              active ? "bg-primary-50 dark:bg-primary-900/20 scale-110" : "bg-transparent"
            )}>
