...
export default function LiabilityPage() {
  ...
  const entriesWithBalance = useMemo(() => {
    ...
    const filtered = filterByStartDate(liabilityEntries, settings.startDate)
  ...
  const dashStats = useMemo(() => {
    const all = filterByStartDate(liabilityEntries, settings.startDate);
    ...
    const catStats = liabilityCategories.map(c => {
      const cEntries = all.filter(e => e.categoryId === c.id);
    ...
    return { globalNet, catStats };
  }, [liabilityEntries, liabilityCategories, settings.startDate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const payload = { ...form, debit: parseFloat(form.debit) || 0, credit: parseFloat(form.credit) || 0, categoryId: selectedCat };
      if (editingEntity) {
        await updateLiabilityEntry(editingEntity.id, payload);
        toast('Updated', 'success');
      } else {
        await addLiabilityEntry({ ...payload, balance: 0 });
        toast('Added', 'success');
      }
      closeForm();
    } catch (err) {
  ...
  const handleEdit = (e: any) => { setEditingEntity(e); setForm({ date: e.date, description: e.description || '', debit: e.debit.toString(), credit: e.credit.toString() }); setShowEntryForm(true); setViewingEntity(null); };
  ...
  return (
    <div className="animate-fade-in flex flex-col h-full w-full overflow-hidden text-slate-900 dark:text-white">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        {activeTab === 'analytics' ? (
          <div className="p-4 space-y-6">
            <div className="glass-card !bg-orange-600 border-none shadow-orange-600/30">
               <p className="text-[10px] font-black text-white/70 uppercase tracking-widest mb-1">TOTAL NET LIABILITY</p>
               <h2 className="text-4xl font-[900] text-white">₨ {formatCurrency(Math.abs(dashStats.globalNet))}</h2>
               <div className="mt-4 pt-4 border-t border-white/10 flex justify-between">
                  <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">{liabilityCategories.length} ACCOUNTS</span>
                  <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">{dashStats.globalNet >= 0 ? 'DR BALANCE' : 'CR BALANCE'}</span>
               </div>
            </div>

            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">ACCOUNT BREAKDOWN</h3>
              {dashStats.catStats.map(c => (
                <div key={c.id} onClick={() => { setSelectedCat(c.id); setActiveTab('entries'); }} className="glass-card flex items-center justify-between active:scale-[0.98] transition-transform">
                   <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-900/10 flex items-center justify-center border border-orange-100 dark:border-orange-900/20">
                         <Landmark className="w-5 h-5 text-orange-600" />
                      </div>
                      <div className="flex flex-col gap-0.5">
                         <p className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight">{c.name}</p>
                         <p className={cn("text-[9px] font-bold uppercase tracking-widest", c.balance >= 0 ? "text-slate-400" : "text-red-500")}>
                   ...
                   <div className="text-right flex flex-col items-end">
                      <div className="h-1.5 w-24 bg-slate-100 dark:bg-dark-800 rounded-full overflow-hidden">
                         <div className="h-full bg-orange-500 rounded-full" style={{ width: `${Math.min(100, (Math.abs(c.balance) / (Math.abs(dashStats.globalNet) || 1)) * 100)}%` }} />
                      </div>
                      <p className="text-[9px] font-black text-orange-600 mt-1 uppercase">LIABILITY WEIGHT</p>
                   </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 bg-white dark:bg-dark-900/50 border-b border-slate-100 dark:border-dark-800 space-y-3 shrink-0">
               <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                  {liabilityCategories.map(c => (
                    <button key={c.id} onClick={() => setSelectedCat(c.id)} className={cn("shrink-0 px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", selectedCat === c.id ? "bg-orange-600 text-white shadow-lg" : "bg-white dark:bg-dark-800 text-slate-500 border border-slate-200 dark:border-dark-700")}>
                      {c.name}
                    </button>
                  ))}
               </div>
               <SearchBar value={search} onChange={setSearch} placeholder="SEARCH TRANSACTIONS..." fullWidth />
            </div>
          ...
          <div className="p-6">
            <div className="glass-card space-y-6">
              <h2 className="text-xl font-black uppercase tracking-tight">New Liability Account</h2>
              <form onSubmit={handleAddCategory} className="space-y-4">
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black text-orange-600 uppercase tracking-widest px-1">Account Name</label>
                   <input className="input w-full !h-12" value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. BANK LOAN A" required dir="auto" />
                </div>
                <button type="submit" className="w-full py-4 bg-orange-600 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-orange-600/20 active:scale-95 transition-all">
                  REGISTER ACCOUNT
                </button>
              </form>
            </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              {liabilityCategories.filter(c => !manageSearch || c.name.toLowerCase().includes(manageSearch.toLowerCase())).map(c => (
                   ...
                   {editingId === c.id ? (
                     ...
                     <>
                        ...
                        <div className="flex items-center gap-1">
                           ...
                           {currentUser?.role === 'Admin' && <button onClick={() => { if(confirm('Delete account?')) deleteLiabilityCategory(c.id); }} className="p-2 text-red-400"><Trash2 className="w-4 h-4" /></button>}
                        </div>
                     </>
                   )}
                </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest px-1">Debit (Paid)</label><input type="number" step="any" className="input w-full !h-12 !text-emerald-600" value={form.debit} onChange={e => setForm({ ...form, debit: e.target.value })} placeholder="0.00" /></div>
                <div className="space-y-1.5"><label className="text-[10px] font-black text-red-600 uppercase tracking-widest px-1">Credit (Owed)</label><input type="number" step="any" className="input w-full !h-12 !text-red-600" value={form.credit} onChange={e => setForm({ ...form, credit: e.target.value })} placeholder="0.00" /></div>
              </div>
            </div>
            <button type="submit" disabled={isSaving} className="w-full py-4 bg-orange-600 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-orange-600/20 active:scale-95 transition-all">
      ...
      {(activeTab === 'analytics' || activeTab === 'entries') && (
        <FAB icon={Plus} onClick={() => { if(liabilityCategories.length === 0) setActiveTab('register'); else setShowEntryForm(true); }} label="NEW" className="!bg-orange-600 shadow-orange-600/40" />
      )}

      {viewingEntity && <TransactionReceiptModal entity={viewingEntity} type="liability" onClose={() => setViewingEntity(null)} onEdit={handleEdit} onDelete={() => deleteLiabilityEntry(viewingEntity.id)} />}
      {showReport && <PrintReportModal data={entriesWithBalance} type="liability" onClose={() => setShowReport(false)} />}
    </div>
  );

  function handleAddCategory(e: React.FormEvent) {
    e.preventDefault(); if (!newName.trim()) return;
    if (liabilityCategories.some(c => c.name.toLowerCase() === newName.trim().toLowerCase())) { toast('Already exists', 'error'); return; }
    addLiabilityCategory(newName.trim()); setNewName(''); toast('Account registered', 'success'); setActiveTab('analytics');
  }

  function handleSaveEdit(id: string) {
    if (!editForm.name.trim()) return;
    if (liabilityCategories.some(c => c.id !== id && c.name.toLowerCase() === editForm.name.trim().toLowerCase())) { toast('Already exists', 'error'); return; }
    updateLiabilityCategory(id, editForm.name.trim()); setEditingId(null); toast('Updated', 'success');
  }
}
