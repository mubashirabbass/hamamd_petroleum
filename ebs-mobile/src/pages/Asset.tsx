...
export default function AssetPage() {
  ...
  const entriesWithBalance = useMemo(() => {
    ...
    const filtered = filterByStartDate(assetEntries, settings.startDate)
  ...
  const dashStats = useMemo(() => {
    const all = filterByStartDate(assetEntries, settings.startDate);
    ...
    const catStats = assetCategories.map(c => {
  ...
  }, [assetEntries, assetCategories, settings.startDate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingEntity) {
        ...
        await addAssetEntry({ ...payload, balance: 0 });
        toast('Added', 'success');
      }
      closeForm();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in flex flex-col h-full w-full overflow-hidden text-slate-900 dark:text-white">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        {activeTab === 'analytics' ? (
          <div className="p-4 space-y-6">
            <div className="glass-card !bg-emerald-600 border-none shadow-emerald-600/30">
               <p className="text-[10px] font-black text-white/70 uppercase tracking-widest mb-1">NET ASSET VALUATION</p>
               <h2 className="text-4xl font-[900] text-white">₨ {formatCurrency(Math.abs(dashStats.globalNet))}</h2>
               <div className="mt-4 pt-4 border-t border-white/10 flex justify-between">
                  <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">{assetCategories.length} ACCOUNTS</span>
                  <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">{dashStats.globalNet >= 0 ? 'STATUS: POSITIVE' : 'STATUS: NEGATIVE'}</span>
               </div>
            </div>
              ...
              {dashStats.catStats.map(c => (
                <div key={c.id} onClick={() => { setSelectedCat(c.id); setActiveTab('entries'); }} className="glass-card flex items-center justify-between active:scale-[0.98] transition-transform">
                   <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 flex items-center justify-center border border-emerald-100 dark:border-emerald-900/20">
                   ...
                   <div className="text-right flex flex-col items-end">
                      <div className="h-1.5 w-24 bg-slate-100 dark:bg-dark-800 rounded-full overflow-hidden">
                         <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.min(100, (Math.abs(c.balance) / (Math.abs(dashStats.globalNet) || 1)) * 100)}%` }} />
                      </div>
                      <p className="text-[9px] font-black text-emerald-600 mt-1 uppercase">VALUATION SHARE</p>
                   </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 bg-white dark:bg-dark-900/50 border-b border-slate-100 dark:border-dark-800 space-y-3 shrink-0">
               <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                  {assetCategories.map(c => (
                    <button key={c.id} onClick={() => setSelectedCat(c.id)} className={cn("shrink-0 px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", selectedCat === c.id ? "bg-emerald-600 text-white shadow-lg" : "bg-white dark:bg-dark-800 text-slate-500 border border-slate-200 dark:border-dark-700")}>
                      {c.name}
                    </button>
                  ))}
               </div>
          ...
          <div className="p-6">
            <div className="glass-card space-y-6">
              <h2 className="text-xl font-black uppercase tracking-tight">New Asset Account</h2>
              <form onSubmit={handleAddCategory} className="space-y-4">
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest px-1">Asset Name</label>
                   <input className="input w-full !h-12" value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. GENERATOR A" required dir="auto" />
                </div>
                <button type="submit" className="w-full py-4 bg-emerald-600 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-emerald-600/20 active:scale-95 transition-all">
                  REGISTER ACCOUNT
                </button>
              </form>
            </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              {assetCategories.filter(c => !manageSearch || c.name.toLowerCase().includes(manageSearch.toLowerCase())).map(c => (
                   ...
                   {editingId === c.id ? (
                     ...
                     <>
                        ...
                        <div className="flex items-center gap-1">
                           ...
                           {currentUser?.role === 'Admin' && <button onClick={() => { if(confirm('Delete account?')) deleteAssetCategory(c.id); }} className="p-2 text-red-400"><Trash2 className="w-4 h-4" /></button>}
                        </div>
                     </>
                   )}
                </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest px-1">Debit (+)</label><input type="number" step="any" className="input w-full !h-12 !text-emerald-600" value={form.debit} onChange={e => setForm({ ...form, debit: e.target.value })} placeholder="0.00" /></div>
                <div className="space-y-1.5"><label className="text-[10px] font-black text-red-600 uppercase tracking-widest px-1">Credit (-)</label><input type="number" step="any" className="input w-full !h-12 !text-red-600" value={form.credit} onChange={e => setForm({ ...form, credit: e.target.value })} placeholder="0.00" /></div>
              </div>
            </div>
      ...
      {(activeTab === 'analytics' || activeTab === 'entries') && (
        <FAB icon={Plus} onClick={() => { if(assetCategories.length === 0) setActiveTab('register'); else setShowEntryForm(true); }} label="NEW" className="!bg-emerald-600 shadow-emerald-600/40" />
      )}

      {viewingEntity && <TransactionReceiptModal entity={viewingEntity} type="asset" onClose={() => setViewingEntity(null)} onEdit={handleEdit} onDelete={() => deleteAssetEntry(viewingEntity.id)} />}
      {showReport && <PrintReportModal data={entriesWithBalance} type="asset" onClose={() => setShowReport(false)} />}
    </div>
  );

  function handleAddCategory(e: React.FormEvent) {
    e.preventDefault(); if (!newName.trim()) return;
    if (assetCategories.some(c => c.name.toLowerCase() === newName.trim().toLowerCase())) { toast('Already exists', 'error'); return; }
    addAssetCategory(newName.trim()); setNewName(''); toast('Account registered', 'success'); setActiveTab('analytics');
  }

  function handleSaveEdit(id: string) {
    if (!editForm.name.trim()) return;
    if (assetCategories.some(c => c.id !== id && c.name.toLowerCase() === editForm.name.trim().toLowerCase())) { toast('Already exists', 'error'); return; }
    updateAssetCategory(id, editForm.name.trim()); setEditingId(null); toast('Updated', 'success');
  }
}
